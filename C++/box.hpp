...
struct box 
{
	...
	string doIsolatedCommand (string cmd, string timelimit, string memorylimit, string in, string out)
	{
		...
		ss << "docker run -v " << this->getPath () << ":/foo -w /foo -i --read-only -m " << memorylimit << " --network none alextsvetanov/atjudgeimage:lastest /bin/bash -c \'cat " << in << " | timeout " << timelimit << " " << cmd << "\' > " << out << " ; echo \"$?\" > log ;";
